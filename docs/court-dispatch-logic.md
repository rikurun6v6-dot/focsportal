...
score = roundScore            // (MAX_ROUND - round + 1) * round_weight … 下位ラウンド優先（水平進行）
      ...
      + divisionBonus
      + categoryBoost
      + bracketOrderBonus      // 同ラウンド内は「表の自然な順（左上→左下→右上→右下）」
```

- **bracketOrderBonus**（`calcBracketOrderBonus`）: 同一ラウンド内の出場順を制御。
  - `bracket_order`（0始まり・上ほど小）＋ `bracket_order_count`（そのラウンドの試合数）で
    「ラウンド内順位 0〜1」に正規化し、先頭=`+BRACKET_ORDER_BONUS_MAX(=60)`、末尾=`+0`。
  - **トーナメント規模（8/16/32試合）に依らず最大差は常に60点**で一定（固定倍率にしない）。
  - 60点はラウンド境界（`round_weight=100`）を超えず**ラウンド優先を壊さない**ため。
  - 優先順位の目安: `ラウンド優先(100/段) > 連戦回避(-200)/部門優先(+150) > ブラケット順(最大60) > 短時間の待機差`。
  - `bracket_order` 未設定の旧データは `match_number` ベースの極小フォールバック（規模差を作らない）。
  - `bracket_order`/`bracket_order_count` は**生成時**（`TournamentGenerator` 両 knockout パス）に保存。dispatcher で位置を推測しないので壊れにくい。
