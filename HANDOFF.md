...
## fix/resubscribe-on-listener-drop

- 日付: 2026-09-01
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/resubscribe-on-listener-drop`

### 変更内容

`src/lib/firestore-helpers.ts` の `subscribeToCollection` と `subscribeToDocument` を、
購読が切れたら張り直すように変えた。待ち時間は 1秒 → 2 → 5 → 10 → 20 → 30秒 と広げる。
一度でも正常に受け取れたら、待ち時間は1秒に戻す。

### 変更理由

`onSnapshot` はエラーコールバックを1度呼ぶと、そこで購読を終える。
これまではログを出すだけで張り直していなかったため、一度切れると画面が古いまま止まる。

通しリハーサル中、matches の購読が数分おきに `failed-precondition` で切れた
（23:01, 23:02, 23:04 …）。その結果、コート状況の一覧は全コート「フリー」と表示しているのに、
画面上部の呼出通知には試合が出ている、という食い違いが起きた。
コートに試合が入っているのに結果入力欄が出ないので、**当日この状態になると進行が止まる。**
リロードするまで直らない。

`isOfflineError` は `unavailable`（通信断）も同じ扱いにしている。
体育館の回線が一瞬切れただけでも同じ結果になる。

### 影響範囲

- `src/lib/firestore-helpers.ts` の購読ラッパー2つ。**Firestore 関連のため要オーナー承認**
- これらを使う全画面（管理画面・参加者画面・モニター）
- 正常時の動作は変わらない。エラー時に張り直す経路が増えるだけ
- 返り値の解除関数は、張り直し中のタイマーも止めるようにした

### 注意点

- 切れる根本原因（`failed-precondition`）は特定できていない。
  IndexedDB の永続キャッシュ（`persistentLocalCache`）まわりが疑わしいが、
  大会前に触るのは危険なので、今回は「切れても戻る」ようにするだけにした。
- 張り直しの様子は Console の「購読が切れました」で追える。

## fix/orphaned-match-selfheal

- 日付: 2026-09-01
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/orphaned-match-selfheal`

### 変更内容

`src/lib/dispatcher.ts` の `autoDispatchAll` に、巡回のたびの自己修復を追加した。
status が calling / playing なのに、どのコートもその試合を持っていない場合、
その試合を waiting に戻し `court_id` を消す。

### 変更理由

通しリハーサル中に完全な行き詰まりが起きた。

```
試合 t_GbhaIEMoQYdZsMGNZjZl  男子D 1部 B組
  status = calling / court_id = court_..._1
  → どのコートの current_match_id もこの試合を指していない
```

この状態になると、その試合の選手がずっと「試合中」の扱いになり、
同じ選手が出る残り4試合が二度と割り当てられない。
コート状況にカードが出ないので、その画面からは終わらせられない。
アサイン診断には「🔴 選手が試合中」とだけ出て、原因が分からない。

割り当て自体は `claimCourtForMatch` が1つのトランザクションでコートと試合の
両方を書くので、正常な経路ではこの状態にならない。通信断や手動操作で
片方だけ残った場合に発生する。原因の特定より先に、戻せるようにした。

### 影響範囲

- `src/lib/dispatcher.ts` の `autoDispatchAll` のみ
- 休憩の予約は `status: waiting` のままなので対象外。誤って戻すことはない
- 団体戦は複数コートが同じ試合IDを持つが、いずれかが持っていれば対象外
- 空きコートが1面もないときは、そもそもこの処理まで到達しない（既存の早期 return）

### 注意点

- 応急処置であって、片方だけ残る根本原因は特定できていない。大会後に追う
- 発生したら Console に「コートから外れた進行中の試合を待機に戻します」が出る
- 手で直すなら **結果一覧 → 進行中** に出るので、そこから確定または WO で処理できる

## fix/bye-seed-label-order

- 日付: 2026-09-01
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/bye-seed-label-order`

### 変更内容

`src/components/admin/VisualBracket.tsx` の対戦相手ラベルで、
「ペア番号を出す」判定を「BYE（シード）」判定より前に移した。

### 変更理由

BYE 試合は「シードのペア」と「空き」の組み合わせだが、
`isByeMatch(match)` の判定がペア番号の判定より前にあったため、
番号を持っている側まで「シード（不戦勝）」になっていた。
結果、トーナメント表が **シード対シード** という意味のない表示になる。

当日くじ引き前で選手が入っていない状態（＝本番の現在の状態）で必ず起きる。
選手が入っている間は先に名前が返るので隠れていた。

### 影響範囲

- `src/components/admin/VisualBracket.tsx` の `getOpponentLabel` 相当部分のみ
- 選手が確定している試合の表示は変わらない（先に名前を返す経路が上にあるため）
- BYE の空いている側は従来どおり「シード（不戦勝）」

### 注意点

- 表示の順番は 名前 → 勝者表示 → 予選順位 → **ペア番号** → シード → 未定。
  ここを入れ替えると同じ不具合が戻る。

## fix/prelim-qualifier-label

- 日付: 2026-09-01
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/prelim-qualifier-label`

### 変更内容

`src/components/admin/VisualBracket.tsx`。予選リーグのある部の決勝トーナメント枠に、
「予選通過」「予選通過（シード）」というラベルを出すようにした。

### 変更理由

個人戦①（予選リーグ → 決勝トーナメント）の枠は、くじ番号を持たない。
上がってくるのは予選の順位が確定してからなので、これまでは

- BYE 試合 … 「シード（不戦勝）」対「シード（不戦勝）」
- それ以外 … 「未定」対「未定」

と表示されていた。表を貼り出しても何の枠か読めない。

BYE は進出処理（`GroupRankingManager`）が `player1_id` 側にだけ勝ち上がりを書く。
つまり position 1 が勝ち上がり側、position 2 が空き側。
両方を「シード（不戦勝）」にしていたのが、シード対シードの原因。

### 影響範囲

- `src/components/admin/VisualBracket.tsx` の表示のみ
- 個人戦②（混合・シングルス）はくじ番号を持つので、手前の分岐で返る。変化なし
- 女子1部（予選なし・純トーナメント）もくじ番号を持つので変化なし
- 選手が確定した後は先に名前を返す経路が上にあるので変化なし

### 注意点

- ラベルの優先順は 名前 → 勝者表示 → 予選順位 → ペア番号 → 予選通過/シード → 未定。

## fix/bye-source-label

- 日付: 2026-09-01
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/bye-source-label`

### 変更内容

`src/components/admin/VisualBracket.tsx`。BYE 試合から上がってくる枠のラベルを、
予選のある部では「予選通過（シード）」にした。
...
```
決勝 第1試合   準決勝の勝者   VS   準決勝 第1試合の勝者
```

片方は BYE（採番されないので番号なし）、もう片方は実戦の第1試合。
どちらも「準決勝」なので、どちらがどちらか読めない。

BYE は戦わずに上がる枠なので、「勝者」ではなく枠そのものを出すようにした。

```
決勝 第1試合   予選通過（シード）   VS   準決勝 第1試合の勝者
```

### 影響範囲

- `src/components/admin/VisualBracket.tsx` の表示のみ
- 個人戦②（くじ番号あり）は手前で番号を返すので変化なし
- 選手が確定した後は名前を返すので変化なし
## fix/round-name-prelim-leak

- 日付: 2026-09-02
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/round-name-prelim-leak`

### 変更内容

`src/components/admin/ResultsTab.tsx` の `maxRoundByType` の計算を、
「この合宿の、決勝トーナメントの試合だけ」に絞った。

### 変更理由

コート表示のラウンド名が、予選のある部で1段ずれていた。

`maxRoundByType` は「決勝までの残り段数」を出すための数だが、
予選リーグの試合まで数えていた。予選の round は総当たりの巡目であって段数ではない。
4ペア総当たり（round 1〜3）のある部では max が 3 になり、
決勝（round 1）が `getRoundName(1, 3)` で「準々決勝」と表示されていた。

さらに `campId` で絞っていなかったため、他の合宿（テスト用など）の試合も
同じキー `{種目}_{部}` に混ざっていた。

| 部 | 実際 | 修正前の表示 |
|---|---|---|
| 男子D1部 | 決勝 | 準々決勝 |
| 男子D2部・3部 | 準決勝 → 決勝 | 準々決勝 → 準決勝 |
| 女子D2部・3部 | 決勝 | 準々決勝 |

予選のある5部門すべてでずれる。決勝なのに「準々決勝」と呼び出すことになる。

### 影響範囲

- `src/components/admin/ResultsTab.tsx` の表示のみ
- 予選リーグの試合は `getRoundLabel` の手前で `予選 N組` を返すので影響なし
- 予選のない部（女子1部・混合・シングルス）は元から正しい。変化なし
- トーナメント表（`VisualBracket`）は別の計算をしており、元から正しかった
...
## fix/third-place-loser-propagation

- 日付: 2026-09-02
- 担当者: Claude Code（菊池指示）
- ブランチ: `fix/third-place-loser-propagation`

### 変更内容

`src/lib/firestore-helpers.ts` の `updateMatchResult` に、
準決勝の敗者を3位決定戦の枠へ送る処理（`propagateLoserToThirdPlace`）を追加した。

### 変更理由

3位決定戦は生成時に「空のまま」作られる。生成側のコメントには
「準決勝終了後に管理画面の『3位決定戦を作成』で敗者を流し込む」とあるが、
**その画面はどこにも存在しない**（リポジトリ全体を検索しても該当のUIがない）。

つまり敗者を入れる導線がなく、3位決定戦は選手が入らないまま永久に始まらない。
リハーサルでも、女子D1部の3位決定戦が `p1 なし / p2 なし` のまま残った。

該当するのは3位決定戦のある **女子D1部・混合D2部・混合D3部・1部男子S の4種目**。

### 影響範囲

- `src/lib/firestore-helpers.ts` の `updateMatchResult` のみ
- 3位決定戦が存在しない種目・部では何もしない（`third` が見つからず即 return）
- 準決勝以外のラウンドでも何もしない（`current.round !== third.round - 1` で return）
- 上下の位置は準決勝の `match_number` 順で決める
- 失敗しても結果の確定自体は止めない（console に出すだけ）
