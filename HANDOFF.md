...
- 変更内容:
  ...
  **④ 点数の登録をやめ、大会の形から自動で決める（`lib/match-points.ts` 新規）**
  - 種目設定・トーナメント生成から「基本点数」「ラウンド別点数設定（詳細）」の入力欄を削除。
  - ブロックの人数で点数を変える運用（4人ブロックだけ11点）が設定では表現できず、登録の手間も大きかったため、構造から導出する方式に変更した。
...
- 注意点 / 引き継ぎ事項:
  ...
  - **点数は画面から変えられなくなった。** 例外的な点数にしたい場合は `src/lib/match-points.ts` の3つの定数と2つの関数を直すこと。試合ごとの値は生成時に `matches.points_per_match` に焼き込まれるので、生成済みの試合は再生成しないと変わらない。
