...
export type { CorrectionImpact, CorrectionImpactItem } from './correction-logic';
...
/** M0 の合宿の全試合を一度だけ読み、メモリ上でチェーンを辿る（1クエリ・旧 walkAdvancementChain 相当） */
async function loadChain(m0: Match) {
  const matches = await getAllDocuments<Match>('matches', m0.campId ? [where('campId', '==', m0.campId)] : []);
  return traceAdvancementChain(m0, matches);
}
...
export async function analyzeCorrectionImpact(
  ...
  if (!m0) return empty;
  const matches = await getAllDocuments<Match>('matches', m0.campId ? [where('campId', '==', m0.campId)] : []);
  return buildCorrectionImpact(m0, newWinnerId, matches);
}
...
export async function applyRenameChain(
  ...
  try {
    ...
    const { adv, other, items } = await loadChain(m0);
...
export async function cancelMatchResultChain(
  ...
  try {
    const m0 = await getDocument<Match>(COLLECTIONS.matches, matchId);
    if (!m0) return { success: false, error: '試合が見つかりません' };
    const { items } = await loadChain(m0);
    if (items.some(it => it.match.status === 'calling' || it.match.status === 'playing')) {
