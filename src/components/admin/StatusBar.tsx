...
import { Wifi, WifiOff, Clock, ChevronUp, Database, Activity, Sparkles, PauseCircle } from "lucide-react";
import { subscribeToCollection, subscribeToDocument } from "@/lib/firestore-helpers";
...
export default function StatusBar({ isOnline }: StatusBarProps) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (remaining === 0) { setPauseUntil(null); setPauseLabel(''); }
    };
    update();
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100]">
        ...
        <div className="absolute bottom-full mb-2 right-0 bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-200 p-4 min-w-[280px] animate-in slide-in-from-bottom-2 duration-200">
          <div className="space-y-3">
            ...
            {pauseUntil && pauseUntil > new Date() && (
              <div className="flex items-start gap-3 bg-amber-50 -mx-1 px-1 py-2 rounded-lg border border-amber-200">
                <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center">
                  <PauseCircle className="w-4 h-4 text-amber-600 animate-pulse" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold text-amber-800">一時中断中</div>
                  <div className="text-xs text-amber-700 mt-0.5 font-medium">{pauseLabel}</div>
                  <div className="text-xs text-amber-600 mt-0.5">
                    残り <span className="font-mono font-bold">
                      {Math.floor(remainingPauseSeconds / 60).toString().padStart(2, '0')}:{(remainingPauseSeconds % 60).toString().padStart(2, '0')}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <button
        ...
        className={`backdrop-blur-sm border rounded-full px-4 py-2 shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 ${pauseUntil && pauseUntil > new Date() ? 'bg-amber-100/95 border-amber-400' : 'bg-slate-100/95 border-slate-300'}`}
      >
            ...
            <>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-amber-700">
                  {Math.floor(remainingPauseSeconds / 60).toString().padStart(2, '0')}:{(remainingPauseSeconds % 60).toString().padStart(2, '0')} 中断中
                </span>
              </div>
